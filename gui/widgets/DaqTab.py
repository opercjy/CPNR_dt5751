...
class DaqTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.spin_scan_start = QSpinBox(); self.spin_scan_start.setRange(0, 16383); self.spin_scan_start.setValue(500)
        self.scan_layout.addWidget(self.spin_scan_start)
        self.scan_layout.addWidget(QLabel("End:"))
        self.spin_scan_end = QSpinBox(); self.spin_scan_end.setRange(0, 16383); self.spin_scan_end.setValue(600)
        self.scan_layout.addWidget(self.spin_scan_end)
        self.scan_layout.addWidget(QLabel("Step:"))
        self.spin_scan_step = QSpinBox(); self.spin_scan_step.setRange(1, 1000); self.spin_scan_step.setValue(5)
    ...
    def load_settings(self):
        ...
        if saved_config:
            self.parse_env_from_config(saved_config)
    ...
    def browse_output(self):
        ...
        if path: 
            self.output_input.setText(os.path.relpath(path, self.bin_dir))
            self.save_settings()
    ...
    def run_single_batch(self):
        ...
        # 🌟 수집 직전, UI의 환경 변수를 .conf 파일에 동기화 기록
        ...
        if self.env_data_provider:
            current_env_data.update(self.env_data_provider())
        ...
        cmd = ["./frontend_dt5751", "-c", config_full, "-o", output_file]
