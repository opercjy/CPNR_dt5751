...
DAQManager::DAQManager(const std::string &config_file, const std::string &output_file, 
  ...
  if (!output_file_.empty()) {
    static std::vector<char> write_buffer(8 * 1024 * 1024); 
...
void DAQManager::AcquisitionLoop(std::atomic<bool>& is_running) {
  ...
  while (is_running) {
    ...
    if (err != CAEN_DGTZ_Success || bsize == 0) continue; 
    ...
    // 🌟 [수정] 좁은 모니터를 위한 극강의 압축 출력 (80자 제한 통과)
    if (elapsed_ms >= 500.0) {
      ...
      std::cout << "\r\033[K\033[1;36m[DAQ]\033[0m "
